...
числа и строки. При вводе пользователем очередного элемента
...
if __name__ == '__main__':
    ...
    while True:
        user_input = input(
            "Введите число, чтобы добавить в список или 'stop' для выхода: ")
        if user_input == "stop":
            break
        try:
            if not user_input.isdigit():
                raise NonNumberError(
                    f"'{user_input}' нецифровой формат")
            my_list.append(int(user_input))
        except NonNumberError as e:
            print(e)
